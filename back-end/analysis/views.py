...
class Dong_AI(APIView):
    def get(self, request, *args, **kargs):
       ...
       '시간대_14_17_유동인구_수', '시간대_17_21_유동인구_수', '시간대_21_24_유동인구_수','분기', '코로나_여부', '전년도_점포별_평균_매출_금액']
        ...
        queryset_estimate = DongData.objects.filter(기준_년분기_코드 = 20232, 행정동_코드_명 = dong, 서비스_업종_코드_명 = business).values(*x_cols_estimate[:-1])
       ...
       '코로나_여부', '전년도_점포별_평균_매출_금액']
        ...
        queryset_predict = DongData.objects.filter(기준_년분기_코드 = 20233, 행정동_코드_명 = dong, 서비스_업종_코드_명 = business).values(*x_cols_predict[:-1])
        ...
        return Response({"행정구":goo, "행정동":dong, "업종" : business, "자본금" : funds, "23년도 3분기 추정" : estimate_result[0] , "23년도 4분기 예측" : predict_result[0] }, status=status.HTTP_200_OK)

...
const Board2View = () => {
  ...
  function getBoardContent() {
    axios.get(`https://3.38.190.216/board/postlist/${post_num}`)
      .then((response) => {
  ...
  function getcomment() {
    axios.get(`https://3.38.190.216/board/postlist/${post_num}/comment`)
      .then((response) => {
  ...
  const handleCommentInput = () => {
    ...
    axios.post(`http://3.38.190.216/board/postlist/${post_num}/createcomment`, {
      contents,
      name,
    })
  ...
  const handleCommentDelete = (commentId) => {
    axios.delete(`http://3.38.190.216/board/postlist/deletecomment/${commentId}`)
      .then((response) => {
        console.log(response.data);
  ...
  const handleDelete = () => {
    axios.delete(`http://3.38.190.216/board/postlist/${post_num}/deletepost`)
      .then((response) => {
        console.log(response.data);
        // 삭제 성공 시 리다이렉트 또는 필요한 동작 수행
        navigate('/board2');
    ...
    <Box sx={{ p: 2, height: '100%' }}>
      ...
      {comments.map((c) => (
        ...
        <Typography variant="body1" fontSize={15} sx={{ mb: 3, display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span>{c.name} {c.date} 
            <Button size='small' onClick={() => handleCommentDelete(c.comment_id)} href={`/board2View/${post_num}`} sx={{ ml: 1 }}>
              X
            </Button>
          </span>
        </Typography>

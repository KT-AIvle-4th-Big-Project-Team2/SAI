...
const Board1View = () => {
  ...
  const handleCommentInput = () => {
    ...
    axios.post(`https://3.38.190.216/board/postlist/${post_num}/createcomment`, {
      contents,
      name,
    })
  ...
  const handleCommentDelete = (commentId) => {
    axios.delete(`https://3.38.190.216/board/postlist/deletecomment/${commentId}`)
      .then((response) => {
        console.log(response.data);
  ...
  const handleDelete = () => {
    axios.delete(`https://3.38.190.216/board/postlist/${post_num}/deletepost`)
      .then((response) => {
        console.log(response.data);
        // 삭제 성공 시 리다이렉트 또는 필요한 동작 수행
        navigate('/Board1');
